  ...
  'fixed z-50 flex h-full flex-col border-border bg-surface shadow-lift duration-base data-[state=open]:animate-in data-[state=closed]:animate-out',
  {
    variants: {
      side: {
        ...
        left: 'inset-y-0 left-0 w-full max-w-[min(400px,100vw)] border-r data-[state=closed]:slide-out-to-left data-[state=open]:slide-in-from-left',
      },
    },
    defaultVariants: {
      side: 'right',
...
const AsideHeader = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div
    className={cn(
...
const AsideBody = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div className={cn('flex-1 overflow-y-auto p-md', className)} {...props} />
);
AsideBody.displayName = 'AsideBody';

const AsideFooter = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div className={cn('shrink-0 border-t border-border p-md', className)} {...props} />
);
